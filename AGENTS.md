...
单文件 `proxy.mjs`（约 1600 行），包含 HTTP 服务器、协议转换、session 管理、streaming 全部逻辑。
...
- **`test`** — 日常开发分支，所有 commit 在此提交。**永远不要在 master 上直接 commit**。
- **`master`** — 稳定发布分支。只通过 `git merge --squash test` 合并，不直接修改。

```bash
# 提交前确认
git branch --show-current  # 必须是 test
# 合并到 master
git checkout master
git merge --squash test
git commit -m "fix(proxy): summary of all test changes"
```
...
- **Session**: 12h 到期 + 1h 随机抖动，按 API Key 分 session
- **流式超时**: 流式 30s，非流式 90s 无新数据自动中断
- **断连处理**: `res.on('close')` + `AbortController` 打断 CC 上游，抢发 usage=0 终止 chunk
- **零输出防护**: `outputTokens === 0` → 返回 502 错误（反异常计费）
- **连续超时阈值**: 连续 3 次超时才提示"压缩上下文"，任意成功重置
- **超时重试**: 流式超时用 `res.destroy()` 发 RST，触发客户端自动重试
- **隐私保护**: 日志不含 API Key 片段、CC error body、stack trace
