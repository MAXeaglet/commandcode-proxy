...
| HTTP 状态 | 说明 |
|-----------|------|
| 400 | 请求格式错误，或上游 error 事件自带 `statusCode: 400`（如内容风控拒绝） |
...
| 502 | CC 上游错误——事件未声明状态码，或声明的状态码不在映射表内 |

CC 把失败放在响应流里上报，而不是用 HTTP 状态：`{"type":"error","error":{...,"statusCode":N,"isRetryable":B}}`。代理会取这个状态（消息里的 `<NNN>` 前缀优先，与 CLI 一致），按与 HTTP 层错误相同的方式映射（声明的 402 仍落 429），所以上游声明的 400 / 429 会带着自己的状态到达客户端，而不是一律 502。
